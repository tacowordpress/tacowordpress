...
class Base {
  ...
  public function renderAdminColumn($column_name, $item_id) {
    ...
    if(is_array($field)) {
      ...
      if(isset($field['type'])) {
        switch($field['type']) {
          case 'checkbox' : $out = ($entry->get($column_name)) ? __('Yes') : __('No'); break;
          case 'image'    : $out = Html::image($entry->get($column_name), $entry->get($column_name), array('class'=>'thumbnail')); break;
          case 'select'   :
